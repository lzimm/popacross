...
public abstract class ClassWalkerFilter {
	...
	public static ClassWalkerFilter implementingWithParam(final Class<?> c, final Class<?> p) { return implementingWithParam(c, p, new HashSet<Class<?>>()); }
	public static ClassWalkerFilter implementingWithParam(final Class<?> c, final Class<?> p, final Class<?> ... excluding) { return implementingWithParam(c, p, new HashSet<Class<?>>(Arrays.asList(excluding))); }
	public static ClassWalkerFilter implementingWithParam(final Class<?> c, final Class<?> p, final Set<Class<?>> excluding) {
		return new ClassWalkerFilter() {
			@Override
			public boolean visit(Class<?> cls) {
				try {
					if (!excluding.contains(cls) && !c.equals(cls) && c.isAssignableFrom(cls)) {
						for (Type i: cls.getGenericInterfaces()) {
							try {
								ParameterizedType t = (ParameterizedType) i;
								if (t.getRawType().equals(c) && Reflection.getParamType(i, 0).equals(p)) {
									return true;
								}
							} catch (Exception e) {}
						}
